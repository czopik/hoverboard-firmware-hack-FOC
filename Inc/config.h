...
#define BAT_CALIB_REAL_VOLTAGE  1400      // input voltage measured by multimeter (multiplied by 100). In this case 43.00 V * 100 = 4300
#define BAT_CALIB_ADC           400      // adc-value measured by mainboard (value nr 5 on UART debug output)
...
#define BAT_LVL5                (120 * BAT_CELLS * BAT_CALIB_ADC) / BAT_CALIB_REAL_VOLTAGE    // Green blink:  no beep
#define BAT_LVL4                (110 * BAT_CELLS * BAT_CALIB_ADC) / BAT_CALIB_REAL_VOLTAGE    // Yellow:       no beep
#define BAT_LVL3                (100 * BAT_CELLS * BAT_CALIB_ADC) / BAT_CALIB_REAL_VOLTAGE    // Yellow blink: no beep 
#define BAT_LVL2                (90 * BAT_CELLS * BAT_CALIB_ADC) / BAT_CALIB_REAL_VOLTAGE    // Red:          gently beep at this voltage level. [V*100/cell]. In this case 3.60 V/cell
#define BAT_LVL1                (80 * BAT_CELLS * BAT_CALIB_ADC) / BAT_CALIB_REAL_VOLTAGE    // Red blink:    fast beep. Your battery is almost empty. Charge now! [V*100/cell]. In this case 3.50 V/cell
#define BAT_DEAD                (70 * BAT_CELLS * BAT_CALIB_ADC) / BAT_CALIB_REAL_VOLTAGE    // All leds off: undervoltage poweroff. (while not driving) [V*100/cell]. In this case 3.37 V/cell
// ######################## END OF BATTERY ###############################
...
#define TEMP_POWEROFF           6500       // overheat poweroff. (while not driving) [°C * 10]. Here 65.0 °C
// ######################## END OF TEMPERATURE ###############################
...
*/
  ...
  #else
    #define FLASH_WRITE_KEY     0x1001789   // Flash memory writing key. Change this key to ignore the input calibrations from the flash memory and use the ones in config.h
    //#define DEBUG_SERIAL_USART3           // right sensor board cable, disable if I2C (nunchuk or lcd) is used!
  #endif
...
*/
  // #define DUAL_INPUTS                     // ADC*(Primary) + PPM(Auxiliary). Uncomment this to use Dual-inputs
  #ifdef DUAL_INPUTS
    #define FLASH_WRITE_KEY       0x11044  // Flash memory writing key. Change this key to ignore the input calibrations from the flash memory and use the ones in config.h
    //#define CONTROL_ADC           0       // use ADC as input. Number indicates priority for dual-input. Disable CONTROL_SERIAL_USART2, FEEDBACK_SERIAL_USART2, DEBUG_SERIAL_USART2!
  ...
  #else
    #define FLASH_WRITE_KEY       0x10044  // Flash memory writing key. Change this key to ignore the input calibrations from the flash memory and use the ones in config.h
    ...
    #define PRI_INPUT1            3, -500, 0, 500, 100  // TYPE, MIN, MID, MAX, DEADBAND. See INPUT FORMAT section
...
*/
  // #define DUAL_INPUTS                     // ADC*(Primary) + PWM(Auxiliary). Uncomment this to use Dual-inputs
    ...
    #define PRI_INPUT1            3,  0, 0, 1000, 0   // TYPE, MIN, MID, MAX, DEADBAND. See INPUT FORMAT section
    #define PRI_INPUT2            3,  0, 0, 1000, 0   // TYPE, MIN, MID, MAX, DEADBAND. See INPUT FORMAT section
